...
class SequenceSummary(_Model):
    ...
    name: str
    description: Optional[str] = None
    status: Literal["draft", "published", "paused", "archived"] = "draft"
    trigger: Dict[str, Any] = {}
    enrolled_count: int = Field(alias="enrolledCount", default=0)
    published_version: Optional[int] = Field(default=None, alias="publishedVersion")
    updated_at: datetime = Field(alias="updatedAt")


class Sequence(SequenceSummary):
    graph: Dict[str, Any] = {}
    graph_summary: Dict[str, int] = Field(default={}, alias="graphSummary")
    lifecycle_policy: Optional[Dict[str, Any]] = Field(
        default=None, alias="lifecyclePolicy"
    )
    activation_policy: Optional[Dict[str, Any]] = Field(
        default=None, alias="activationPolicy"
    )
...
class SequenceRun(_Model):
    ...
    subject_id: str = Field(alias="subjectId")
    status: Literal[
        "active", "waiting", "blocked", "paused", "completed", "exited", "cancelled", "failed"
    ]
    flow_version: int = Field(alias="flowVersion")
    ...
    wait_until: Optional[Dict[str, Any]] = Field(default=None, alias="waitUntil")
    last_decision: Optional[Dict[str, Any]] = Field(default=None, alias="lastDecision")
...
class EmitEventResult(_Model):
    ...
    status: Literal["accepted", "quarantined", "invalid"] = "accepted"
    resolved_subject_id: Optional[str] = Field(default=None, alias="resolvedSubjectId")
    waits_resumed: List[Dict[str, Any]] = Field(default=[], alias="waitsResumed")
    enrollments: List[Dict[str, Any]] = []
    credits_charged: float = Field(default=0.0, alias="creditsCharged")
