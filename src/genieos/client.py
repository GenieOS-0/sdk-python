"""
Synchronous :class:`GenieOS` client.
...
parallel async client lives in ``genieos.async_client`` and exposes
...
class _Brand(_Resource):
    def list(self) -> builtins.list[t.BrandSummary]:
        body = self._t.request("GET", "/v1/brand")
        items = body if isinstance(body, list) else body.get("data", [])
        return [t.BrandSummary.model_validate(x) for x in items]

    def get(self, brand_id: str = "default") -> t.BrandDetail:
        return t.BrandDetail.model_validate(
            self._t.request("GET", f"/v1/brand/{brand_id}")
        )


class _Connectors(_Resource):
    def catalog(self) -> dict[str, Any]:
        result = self._t.request("GET", "/v1/connectors/catalog")
        return result if isinstance(result, dict) else {}

    def list(self) -> dict[str, Any]:
        result = self._t.request("GET", "/v1/connectors")
        return result if isinstance(result, dict) else {}


class _Pages(_Resource):
    def list(self) -> builtins.list[t.PageSummary]:
        body = self._t.request("GET", "/v1/pages")
        items = body if isinstance(body, list) else body.get("data", [])
        return [t.PageSummary.model_validate(x) for x in items]

    def get(self, id_or_slug: str) -> t.PageDetail:
        return t.PageDetail.model_validate(
            self._t.request("GET", f"/v1/pages/{id_or_slug}")
        )
...
class _Links(_Resource):
    def list(
        ...
        if limit is not None:
            params["limit"] = limit
        body = self._t.request("GET", "/v1/links", params=params)
        items = body if isinstance(body, list) else body.get("data", [])
        return list(items)
    ...
    def utm_suggestions(
        ...
        body = self._t.request("GET", "/v1/links/utm-suggestions", params=params)
        if isinstance(body, dict) and "data" in body:
            return body["data"]
        return body if isinstance(body, dict) else {}
    ...
    def get(self, link_id: str) -> dict[str, Any]:
        ...
        body = self._t.request("GET", f"/v1/links/{link_id}")
        if isinstance(body, dict) and "data" in body:
            return body["data"]
        return body if isinstance(body, dict) else {}
    ...
    def update(self, link_id: str, **fields: Any) -> dict[str, Any]:
        ...
        """
        body = self._t.request("PATCH", f"/v1/links/{link_id}", json=fields)
        if isinstance(body, dict) and "data" in body:
            return body["data"]
        return body if isinstance(body, dict) else {}
    ...
    def analytics(
        ...
        *,
        ...
        body = self._t.request("GET", "/v1/links/analytics", params=params)
        if isinstance(body, dict) and "data" in body:
            return body["data"]
        return body if isinstance(body, dict) else {}
...
class _Qr(_Resource):
    ...
    def update(self, qr_id: str, **fields: Any) -> dict[str, Any]:
        ...
        body = self._t.request("PATCH", f"/v1/qr/{qr_id}", json=fields)
        if isinstance(body, dict) and "data" in body:
            return body["data"]
        return body if isinstance(body, dict) else {}
    ...
    def render(
        ...
        *,
        ...
        body = self._t.request("POST", f"/v1/qr/{qr_id}/render", json=payload)
        if isinstance(body, dict) and "data" in body:
            return body["data"]
        return body if isinstance(body, dict) else {}
...
__all__ = ["GenieOS", "DEFAULT_BASE_URL"]
