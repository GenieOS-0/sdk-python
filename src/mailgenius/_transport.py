"""
Low-level transport for the MailGenius Python SDK.
...
class _TransportBase:
    ...
    def __init__(
        ...
        *,
        ...
        if not api_key:
            raise _errors.MailGeniusAuthError(
                "MailGenius API key is required.", code="missing_api_key", status=None
            )
        ...
        for attempt in range(self._max_retries + 1):
            try:
                ...
                if attempt >= self._max_retries:
                    raise _errors.MailGeniusNetworkError(
                        f"Network error contacting MailGenius: {e}",
                        code="network_error",
                    ) from e
